...
SCRIPT_URL = "https://raw.githubusercontent.com/websentry-ai/claude-code-setup/refs/heads/main/hooks/unbound.py"
...
def append_to_file(file_path: Path, line: str, var_name: str = None) -> bool:
    try:
        ...
        if line + "\n" not in lines and line not in [l.rstrip() for l in lines]:
            has_header = any("# Claude Code Unbound Configuration" in l for l in lines)
            
            if not has_header:
                lines.append("\n# Claude Code Unbound Configuration\n")
            lines.append(f"{line}\n")
